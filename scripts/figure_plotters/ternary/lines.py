...
def ticks(ax, scale, ticks=None, locations=None, multiple=1, axis='b',
          offset=0.015, clockwise=False, axes_colors=None, textsize=6, **kwargs):
    """
    ...
    if 'r' in axis:
        for index, i in enumerate(locations):
            ...
            ax.text(x, y, str(tick), horizontalalignment="center", 
                color=axes_colors['r'], size=textsize)
    ...
    if 'l' in axis:
        for index, i in enumerate(locations):
            ...
            x, y = project_point(text_location)
            ax.text(x, y, str(tick), horizontalalignment="center",
                color=axes_colors['l'], size=textsize)
    ...
    if 'b' in axis:
        for index, i in enumerate(locations):
            ...
            x, y = project_point(text_location)
            ax.text(x, y, str(tick), horizontalalignment="center",
                color=axes_colors['b'], size=textsize)
