...
// Variables to change
val newSystems = Seq[Seq[String]](Seq("Co","V","Zr"))

// Load in the LB set
val data = UtilityOperations.loadState("../3_with-theories/gfa-training-data.obj").asInstanceOf[Dataset];

// Add in the other data
for (sys <- newSystems) {
    val temp = data.emptyClone();
    temp.importText(s"../datasets/new-data/sputtering_${sys.mkString("")}.data", null);
    data.combine(temp.getRandomSubset(70));
}
